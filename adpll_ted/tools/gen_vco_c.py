...
def var_cell(prefix, tank, vctrl, en, nfin):
    """direct moscap (nfin) + bulk PMOS enable; off-state bulk -> VDD 1M."""
    cb = f"{prefix}_cb"
    lines = [
        f"{prefix}_mv ({tank} {cb} {cb}) {MOSCAP} nfin={nfin}",
        mos(f"{prefix}_sw", vctrl, en, cb, "VDD", 2, model="pch"),
        f"{prefix}_rb ({cb} VDD 0) resistor r=1M",
    ]
    return lines
...
def gen_vco_x():
    ...
    for i in range(12):
        ...
        L.append(cfmom(f"CF{i}", plus, minus, "VSS", 165))
...
def main():
    lines = [
        ...
        "// EN=0 -> bank group enabled (PMOS bulk switch on); EN=0.8 -> disabled",
