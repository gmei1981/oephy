#!/usr/bin/env python3
"""Read the hand-drawn spd_x schematic from the local Virtuoso via bridge.

Dumps full topology (instances + all params + terminal nets + pins) to
tools/spd_x_sch_dump.json and prints a compact summary.
"""
import json
...
data = client.schematic.read(
    "adpll_sch", "spd_x", include_positions=False, param_filters=None
)

out = ROOT / "tools" / "spd_x_sch_dump.json"
...
for inst in data.get("instances", []):
    ...
    # print only non-default params (short list)
