...
- **V4 10µs 整数闭环完美锁定（0 错误 2.6h，服务器孤儿进程手动取回）**：
  - fVCO 各窗口 [1-3/3-6/6-9/9-10µs] = 7.99998/7.99997/8.00010/8.00017G
    ——全程 8.0G ±0.002%（判定阈 ±0.1%）
  - **GS 两档全换**：KP 8→4（~3µs）→2（~6µs）、KI 1→0.5→0.25（锁定判定
    通过才会换档——每 128 REF 窗 bb_dz 全零且 |Σbb|≤4）
  - bb_v 每周期均值 [2.5-5/5-7.5/7.5-9.9µs] = −0.057/−0.065/+0.025（阈值 0.35）✓
  - dz 每周期命中率 98.4%/95.6%/100% ✓；VC1=0.375（码 96）、VC2 游走 0-10mV
    （fine 码 0-5）
  - 抖动（t>5µs 锁后 10000 条 DIV4 边沿）：4 周期 rms 0.509ps、cyc-to-cyc
    0.118ps、单周期等效 254fs（白噪声 /√4 口径）、半窗均值漂移 −3fs（无游走）。
    注意这是**无器件噪声 tran** 的环路诱发抖动（BB 量化+DAC 步进），V6 对比
    论文 196.8fs 时需注明口径差异
  - 工具：tools/parse_psfascii.py（2GB psfascii 单遍流式抽取，比逐信号 psf
    导出快一个量级；坑：Path.with_suffix 只换最后一段后缀）
- **V5 分数通道首跑（8.05G/FCW=401/target=322，1.2µs psfbin 重跑 0 错误）**：
  判定 8/9——AFC 码 112（预期 109-125 ✓）、done@960ns、bb_v=+0.200、dz 40%、
  VC1=0.4358 ✓。**唯一 FAIL（fVCO=8.0398G，−0.127%）是窗口太短而非失锁**：
  VC2=0.498V 满量程 = 环路在 112/113 粗码边界穿越中（AFC 落码 112 → f≈8.012G，
  距 8.05G 差 ~38MHz，fine DAC 全量程 28MHz 拉满后由 DLF 16 位进位接管 coarse
  步进；1.2µs 只拍到拉入中间态，预计 6-8µs 收敛）。
  **结构性问题定位（下次会话处理）**：① AFC 计数粒度 ±1 沿=±25MHz 残差可能
  超出 fine DAC 28MHz 覆盖（整数通道 620kHz 是幸运落点）→ **AFC tol 改 0**（再
  武装游走已支持，命中判据收紧到精确 322）；② **τ_f 低码段非线性**（码 256 处
  偏差 −27ps、码<300 斜率仅 0.03-0.07ps/LSB vs 高码段 0.16）→ 分数抖动残差/
  杂散主源，候选对策=frac 码预失真 LUT 或码基偏置（线性段 [350,1023] 跨度
  109.6ps < 需求 125ps，纯偏置不够，需 LUT）；③ 分数收敛时间 ~6-8µs → 判定
  窗口按 10µs 跑。
  psfxl 坑：25.1 默认输出 psfxl，本地 IC618 psf 和服务器 IC25 psf 都导不出
  数据段 → **手动跑必须显式 -format psfbin**。
