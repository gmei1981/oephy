#!/usr/bin/env python3
"""V4 full closed-loop verdicts (integer 100M/8G).
Lock criteria (plan §5): fVCO 8.0G+-0.1%, BB mean ~0, BB_DZ sustained zero,
AFC done, coarse code ~96 (V3 result), GS gear progression in longer runs.
Usage: python3 tools/check_ssbb_v4.py [rawdir-or-.raw] [lockwin_start_us]
"""
import re
import subprocess
import sys
...
def main():
    ...
    check("afc code ~96", 88 <= cE <= 104, f"end code={cE} (V3 ref 96)")
    ...
    check("fVCO 8.0G+-0.1%", f is not None and abs(f - 8.0e9) < 8e6,
          f"fVCO={f/1e9 if f else None:.5f}G edges={ne}")
