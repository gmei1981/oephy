...
## 2026-09-03 下午：AFC 窗口 off-by-one 修复 + V1 全绿 + V2 开环标定（本会话）

- **AFC 根因（窗口长度 off-by-one）**：窗末计数实测 low=[312,332,331,...]、
  hit=[320,340,...]、high=[328,349,...]，码翻转时刻 160/330/500ns（间隔 170ns）
  ——第 1 窗 160ns 正确，之后每窗 17 拍。机制：评估沿（第 17 REF 沿）处 ref_cnt=0，
  但评估沿到下一窗首沿之间的 1 拍里 div4 沿仍计入新窗 → 窗口 2 起 170ns。
  计数恒高 ~20 沿 → 每窗判"偏多" → low 台码一路下行走停 65（96→80→72→68→66→65），
  hit 台永不命中。修复（netlist/va/pll_afc.va）：首窗 K+1 个沿（从 t=0 跨满 K 拍），
  后续窗 K 个沿（first_win 标志）；同时加 **tol=1 沿容差**（|cnt-target|≤tol 判命中，
  吸收边界计数抖动 312/313 交替——hit_cnt 不清零是有意的：±1 抖动交替下
  "连续命中"判据永不触发）。
- **修复后三台全对**：窗间隔精确 160ns、计数稳定 312/320/328；low 二分
  64→96→112→120→124→126→**127** ✓、high 64→32→16→8→4→2→**1** ✓、
  hit 保持 **64** 且 **AFC_DONE 恰在 960ns 置位**（第 6 窗步长耗尽时 hit_cnt 已 6≥2）。
  **check_ssbb_v1 19/19 ALL PASS**（V1 收口）。
- **V2a VCO KVCO 标定（vco_c_dac 首轮全错→端口陷阱）**：TB 首轮 12 点 0 错误但
  fVCO≈15.5-15.8G、OUTP 摆幅 −0.4~2.3V。根因：**vco_x_c_dac 的 VDD 脚是 tank 供电脚**
  （vco_dual_8g_c 里 Xa(VDDC ...) 正是这么接的），VDDC 脚在子电路内完全悬空（残留端口）。
  首轮把 0.8V 接到 tank → 大摆幅把 moscap 推进 Cmin → f 翻倍。修正接线
  `Xvco (VDDC VDDC ...)`（两供电脚都接 0.5V 节点），12 点重跑中。
  （原理图转换时此端口命名陷阱要记录：symbol 上 VDD 脚语义=核供电。）
- **V2b DTC→BBPD 扫码链**：首跑 12 台全 FAIL——dtc_netlist 漏
  `ahdl_include "pll_dtc_code.va"`（SFE-23 Xdec 未定义，只拷文件没 include）；
  已补，12 码点重跑中。判据：div4 500p/50% 占空、REF 沿对齐 → CKR 延迟 τ∈(50,200)ps
  时 BB=0（div4 采样电平高=CKR 晚）、τ∈(300,337)ps 时 BB=0.8，翻转点 τ≈250ps
  （CODE≈757 折叠点）；BB_DZ 低窗在 CODE≈0±150 与 757±150。
- **V2a KVCO 标定结果（0.5V tank 正确接线后 12 点全过）**：
  **KVCO1=621MHz/V 单调线性**（VC1 0→0.5V：7.8117→7.8685→7.9284→7.9922→8.0563→
  8.1222G，每 0.1V 增 57-66MHz）；**8.0G 目标在 VC1≈0.31 → DAC1 码≈80**（AFC 从
  64 起步可达）✓；**KVCO2≈57MHz/V** 三个基点一致（@VC1=0.1/0.3/0.5：
  55.6/58/59MHz/V）✓；符号为正（VC↑→C↓→f↑，与 wp5 moscap sp 实测一致）。
  CF nr=174 无需重调。DAC1 LSB=3.9mV→2.4MHz/LSB、全量程 310MHz；DAC2 LSB≈1mV→
  55kHz/LSB。AFC tol=±1 沿=±25MHz 残差 → ki=1 拉入最坏 ~50µs（后续可调 ki0/GS）。
- **V2a 测量坑（假二倍频）**：OUTP 波形每周期带一个 ~0.29V 副峰（低 VC1 时半周期
  35/90ps 严重不对称），过零阈值取 0.25V 会双计 → 读出 15.6-16.1G 假频率；阈值
  0.4/0.5/0.6V 实测每周期恰好 1 沿 → **div4 链路无恙**，measure_freq 改用 0.4V。
- **V2b include 顺序坑**：`dtc_10b_ss.scs` 内含 Xdec 实例，`ahdl_include
  "pll_dtc_code.va"` 必须在它之前（SFE-23）；gen_ssbb_top.py 同步修正（全部 VA
  include 先于 scs include）。
- **进程管理事故（教训）**：`pkill -f "dtc_c"` 会匹配自己的 shell 命令行（exit 144
  自杀）且没杀全 → 两个批次的 spectre 写进同一运行目录，日志双段叠加、raw 截断
  在 45ns 但尾部仍是"0 errors"→ 缓存判定误跳过。修复=清掉污染 6 台（c0-c640）
  的 log+raw 重跑；杀后台批次的谱系要用 PID 而不是模式匹配。
- **V2b DTC→BBPD 链验证（12+3 台全过，极性真相）**：dtc_10b_ss 内含 CKXB 反相器
  → **CKR 上升沿=REF 下降沿+τ_f**（非 REF 上升沿）。实测 τ_r（上升沿延迟）
  =32.7→395.5ps 线性 **Tres=0.354ps/LSB** ✓；τ_f（被采样沿）=35→195ps（复位管
  加速下降沿，非对称）。CKR 落 div4 高电平半周（mod 500p 后 35-195ps<250ps）
  → 全码段 BB=0、dz 窗在 τ_f<50ps（码 0-256）——测量自洽，链路健康。
  翻转验证（div4 delay=100p 把折叠窗挪进 τ_f 区间）：**code 300→BB=0.80 ✓**、
  code 500/900→BB=0.00 ✓、dz 窗均按 CKR-沿距正确触发。分数通道余量：τ_f 跨度
  160ps > TVCO 125ps（够用但紧，INL 待标定）。
- **V3 第一轮（2µs 0 错误 8m14s）暴露 AFC 三缺陷**：① 首窗污染——启动瞬态
  （LDO 斜坡+起振）W1 计数 391（应 320），二分首步行反（64→32）；② 步长耗尽
  冻结——爬回 63 时 step=0，|d|=3 残差永不动、afc_done 永不置位；③ 闭环
  VDDC=0.547V（LDO 偏置高 47mV）比 V2a 标定（0.500V）同码低 33MHz → 8.0G 在
  码 ~94 非 80。VDDC 健康 0.547V pp=4.6mV ✓。
- **AFC 二次修复（skipwins=2 + 再武装 + 命中即停步）**：首 2 窗计数但丢弃；
  step 耗尽后 |d|>tol 再武装 step=|d|×lsb_per_cnt(10) clamp 1..32（二分变有界
  游走，保证到 bin），仅在有轨余量方向再武装（码 0/127 停靠不振荡）；命中即
  step=0（停止减半，让 hit 累积）。V1 回归 19/19 全绿（high 台终值 1→0：
  走到轨才算到位，旧停 1 是步长耗尽巧合，check_ssbb_v1 预期已更新）。
- **V3 第二轮全绿（7/7 PASS，2µs 0 错误 mt=8 约 5min）**：AFC 闭环一次二分命中
  ——码轨迹 64→**96**（W3 读 d=−4 → +32 → 96 → cnt=320 命中）、**AFC_DONE@800ns**、
  **fVCO=7.9990G（距 8.0G 目标 10kHz）**、cnt 窗稳定 320、VC1=0.3750=96/256 精确、
  VDDC 0.547V pp=5.8mV。skipwins=2 + 再武装 + VDDC 偏置修正预期（码 96 vs 标定 94）
  全部兑现。
- **V4 组装期架构缺陷（检查发现，未烧仿真）**：原 DLF 只在 done 沿装载 AFC 基址，
  **搜索期间 AFC 码不驱动 VC1**（VC1 由 DLF 自由码控制）→ AFC 二分测的是不随它动
  的频率 → 永不命中、done 永不置位。修复（pll_dlf.va）：afc_done 低电平期间 DLF
  **从动**——acc 实时跟踪 afc_code<<9、bb 积分暂停、code_v=acc（无 kp 路径），
  done 上升沿无缝交接（原装载逻辑保留覆盖 t=0 即 done 的情形）。tb_dlf 相应改
  AFC_DONE=0.8（done=0 现在意味着从动模式），回归 19/19 全绿。
- **V4 1.2µs 冒烟 9/9 ALL PASS（服务器 ax 18.6min）**：fVCO=**7.99938G**（8.0G
  目标 −0.008%）、AFC 码 96/done@800ns、VC1=0.3746、VDDC 0.548、dz 每周期
  100% 命中。**BB 度量口径坑**：bb 是保持电平，时间域均值（0.55）被高低保持时长
  不等加权——必须按 DLF 消费点（REF 上升沿前）逐周期采样：bb_v=+0.154（正在拉
  入 AFC 残差 −620kHz；VC2 已爬 fine 码 4/需 11，与 acc 漂移自洽）。
  check_ssbb_v4 已改逐周期采样口径。
- 服务器侧两坑：① save 语句折行在 25.1 被解析成新实例语句（SFE-23 A0 引用 A6）
  → gen_ssbb_top save 恒单行；② 桥 key 认证又失效（authorized_keys 疑被清，
  第二次）→ sshpass 强制 password 法验证 + 公钥 append 恢复（记忆已更新恢复法）。
- **V4 10µs 长跑已上服务器**（lean save 9 信号 ≈1.7GB；预计 ~2.6h）：看点——
  残差拉入完成（bb_v→0）、GS 换档（W=128，首查 ~2.1µs、二档 ~3.4µs）、
  锁后 6µs 静区 → 周期抖动统计（DIV4 边沿×4 口径）供 V6 对比。

## 2026-09-03 晚：ref3 ADPLL_ALG 架构改造启动——计划定稿 + V0 十四模块全绿（本会话）

- **ref3 资产解析**：`ref3/ADPLL_ALG.docx`（V1.00 内部算法方案说明书）的 18 张框图均为
  内嵌 Visio OLE 对象（word/embeddings/*.vsdx）——解 zip 取 vsdx 再解 visio/pages/page*.xml
  的 <Text> 即得全部信号名/位宽/结构（提取件 ref3/img/、文本 ref3/ADPLL_ALG_extracted.txt），
  EMF 预览图本机无解码器不必转换。架构=MMD 分频反馈型 ADPLL（25M 参考、4.8-5.2G、50kHz
  DCO 步长、TDC 多bit鉴相 + DTC 预测 + 三路后台校准 + 五状态 FSM）。
- **计划定稿**：`ref3_adpll_alg_plan.md`（模块映射表/阶段 R1-R6/风险/决策点 A-E）。
  用户确认按默认方案启动：A=阶段A 先用现有 8G/100M 硬件验证算法（25M/4.8-5.2G 后置）；
  B=VA 行为 TDC 先行；C=Pbank 由 Abank 高位兼任；D=校准按 spec 默认开关；
  E=ref2 线封存（V4 10µs 服务器跑完留档作 BBPD 对照）。
- **V0 落地（14 个 VA，netlist/va/）**：新增 pll_dsm_fb（MASH1/2/3+dither+近整数+q_err+
  sel_clk_fb 延迟线）、pll_mmd_ps（P/S 分频+sel 半周期沿选择）、pll_tdc_beh（cbin=TCKV/4、
  fbin=cbin/16≈1.95ps，ca/cb/f 三组 16b 温度计+arb，±2TCKV 折叠窗）、pll_pfd（前沿1解码+
  tdc_code 9b+phe 归一化+one-hot LUT 回送）、pll_fsm（IDEL→AFC→FLL→PLL→LOCK/UNLOCK）、
  pll_lpf（三级交接：AFC 码→freq_ctrl→phe 闭环；Abank 9b+lpf_frac 26b 拆分；kp/ki 双档）、
  pll_dsm_dco（lpf_frac 26b→4b，中心+8）、pll_fll（2^thr 窗计数+1/2^fstep 积分，freq_ctrl
  为相对残差 LPF 加基址）、pll_lockdet、pll_dac9b、pll_cal_kdtc（phe×q_err LMS，kdtc0=
  TCKV/Tres≈353，dtc_code=kdtc×(q_err+0.5)+dcomp）、pll_cal_refdcc（奇偶选通积分）、
  pll_cal_dcodcc（sign-extended sel 相关→dcomp）；改造 pll_afc6（6b Pbank 版：×8 映射 Abank、
  2^thr 窗、最小误差记录+max_rearm 兜底，skipwins/再武装/窗口 off-by-one 全继承）。
- **V0 判定 PASS**：sim/ref3/v0/tb_compile.scs 把 14 模块连成数字链骨架（REF 100M pulse、
  VCO 8G 正弦→mmd_ps /80→CKFB 100M、CKR=REF+3n、tdc↔pfd one-hot 闭环回接、FSM 全使能链），
  本机 spectre 20.1：**0 errors / 0 warnings**，1.5µs tran 49s 跑完。
- **坑沉淀（本会话）**：① **漏写 endmodule 是系统性错误**（14 个新文件 13 个漏）——VACOMP-2259
  "unexpected EOF or end of line" 报在文件末行之后（EOF 位置）= 缺 endmodule 的特征码，以后
  新写 VA 收尾自查 `end`+`endmodule` 两行；② 复合命令里 cd 不持久 → 相对路径 cp 静默失败，
  批量文件操作用绝对路径；③ 本机 20.1 无 `-logfile` 参数，日志用 `+log <file>`；
  ④ PSFXL 值区本机 psf 工具读不出（VALUE 段空、TRACE 表正常）——编译检查不受影响，
  要取数必须 `-format psfbin` 重跑（与既有记忆一致）。
- **待续（V1）**：单模块 TB（tools/run_ref3_v1.py + check_ref3_v1.py，沿用 ssbb_v1 模式）——
  重点：dsm_fb 阶数/dither 频谱与 q_err 幅度、mmd_ps ratio 4-8191 扫描+sel 半周期相移、
  tdc+phe 线性度（psfbin 取数）、fll/lpf 三级交接无跳变、afc6 二分轨迹、三校准收敛、
  fsm 全状态跳转；随后 V2 开环标定（tdc_norm_coef/kdtc0 定标、Abank 9b KVCO 复核）。

## 2026-09-03 晚续：ref3 V1 单测全绿——22 台 TB 0 错误 + 42/42 判定 ALL PASS（本会话）

- **工具落地**: tools/run_ref3_v1.py（22 台 TB 生成+本机跑台，psfbin 取数）+ tools/check_ref3_v1.py
  （42 项判定）。结果在 sim/ref3/v1/<tb>/。覆盖：dsm_fb(m0/m1 阶数、q_err 幅度、sel 占空比)、
  mmd_ps(r80/r16/r255 周期、sel)、tdc+pfd 链(+100p/+230p/−100p 三点 phe/tcode/arb)、fll(命中
  锁定+方向)、lpf(afc→fll→pll 三级交接 165/273)、dsm_dco(frac0.5→8/9 交替)、afc6(hit done+
  slow/fast 停靠)、三校准(斜率/方向)、lockdet(锁/保持/失锁)、fsm(全状态轨迹含 UNLOCK→FLL→PLL)、
  dac9b。
- **V1 抓出的 5 个 VA 真 bug（全部修复）**:
  1. **fll/afc6 计数口径**: ref3 的 AFC/FLL 计的是固定 ÷16 抽头（ckfb16），计数期望
     = floor(FCW×2^thr/16)（drawing1-2 反馈分频器 + 14bit fb_cnt 印证）；若按 MMD 输出(≈fref)
     计数，1 沿分辨率 = N/窗 ≈ 250MHz，无法分辨 9.6MHz Pbank 步进。窗口 = 2^thr 参考沿
     （afc6 thr=8→256 沿/2.56µs→1 沿=6.25MHz；fll thr=9→512 沿→1 沿=3.1MHz）。
  2. **fll 积分方向**: err>0(f 偏高)→freq_ctrl 下调（V0 版方向反了）。
  3. **mmd_ps 半周期翻转**: 翻转式分频器须每 N/2 沿翻转（高相位 ceil(N/2)、低相位 floor(N/2)）
     才是真 ÷N；V0 的"每 N 沿翻转"输出频率减半（r80 实测 20ns 周期钉死该 bug）。
  4. **afc6 实时输出**: Pbank 码必须实时跟随 code_i（搜索期间每个窗口测的就是当前码的频率）；
     V0 版只在 done 时更新 out_code——闭环里二分永不收敛。fallback 改为 done 时 code_i=best_code。
  5. **fll 窗口长度**: win_edge = pow(2.0,thr) 而非 thr（90ns 窗口/步长 19.64 实锤）。
- **坑沉淀（spectre/工具，重要）**:
  - **sine 源直流偏置参数是 `dc=` 不是 `offset=`**——offset 被静默忽略→正弦以 0 为中心 ±ampl，
    永远不过 +0.4 阈值（mmd 全灭的根因）；
  - **pulse 语义 val1 先行**: val1 占 [delay, delay+width)，val0 在外——设计交替激励相位时必须按
    val1-先行推（calk 符号反的根因；delay=10n 反而与 Q 同相）；
  - **pwl 一次性不重复**——周期交替激励必须用 pulse（val0 可为负）；
  - **real→integer 赋值是四舍五入不是截断**（51.2→51 碰巧同、117.76→118、122.88→123）——需要截断
    处必须显式 floor()；
  - 判定脚本期望值要按激励源精确沿相位推（fll 首窗终于 5121.1ns→计 2561 沿，不是粗算的 2565）；
    生成器产出的网表行首跑要肉眼过一遍（宽度丢单位→width=1 秒→时钟恒高的教训）。
- **V1 判定细节**: fll_hit 首窗 err=+1≤ftol=1 → 首窗即锁（fctrl=−1/128）；afc6_fast 二分
  63→31→15→7→3→1→0 停靠 0 ✓；fsm 状态 1 仅存续 51-201ns、状态 5 仅 1 拍（1201-1211ns），
  判定窗口必须卡准。
- **待续（V2 开环标定）**: tdc_norm_coef/kdtc0 定标（接 dtc_10b_ss 实测 Tres=0.354ps/LSB）、
  Abank 9b KVCO 复核（dac9b+vco_c_dac）、mmd sel 半周期相移链路验证（V1 只验了周期）、
  dither/近整数模式频谱（V1 未覆盖）、FCW=80 整数通道 REF→DTC→TDC 链 phe 摆度。

## 2026-09-03 深夜：ref3 V2 开环标定全绿——12 台 TB 0 错误 + 12/12 判定 ALL PASS（本会话）

- **工具**: tools/run_ref3_v2.py（12 台 TB 并行+缓存）+ tools/check_ref3_v2.py。结果
  sim/ref3/v2/。TB：tdc_sweep（pwl 编程 20 延迟点 10..485p）、mmd_selphase、chain80×5
  （0.7/0.8/0.9n 静态 + 7.9G/8.1G 频率斜坡）、kvco_ab×5（dac9b→vco_c_dac 晶体管）。
- **标定常数（V3 闭环直接使用）**:
  - **TDC**: phe = dtw/TCKV，实测斜率 0.00803/ps（理论 1/125p=0.008，差 0.4%）；逐点 18/20
    在 ±2 LSB 内；折叠窗 ±250p（500p=4 phe 宽）实测正确。**tdc_norm_coef=1/64 默认值成立**。
  - **mmd sel 半周期**: 相移实测 **62.50p**（=TVCO/2 精确）——范围减半机制在链路上成立。
  - **整数通道链方向**: CKR 延迟 +100p → phe −0.81（预期 −0.8）；7.9G/8.1G 频率斜坡方向与
    斜率全对（+0.1001 vs +0.1013/ns；−0.1001 vs −0.0988/ns，误差 <1.5%）。CKFB 输出沿含
    固有 +20.7p 偏移（transition tt/2+计数对齐），闭环由 dtc_offset 吸收。
  - **KVCO_A = 0.607 MHz/LSB**（由 ref2 的 621MHz/V 推算 0.606，差 0.2%），单调；
    f(Abank=0)=7.798G，**8.0G @ Abank≈339**（AFC 码 ≈339/8≈42，在 6b 范围内），
    满量程 511→8.108G。VC1 全程 0~0.5V 工作区（Plan C moscap 单调段）。
  - **kdtc0 = TCKV/Tres = 125p/0.354p ≈ 353.2**（沿用 ref2 V2b 的 DTC τ_r 标定；
    pll_cal_kdtc 默认值不变）。
- **坑沉淀**:
  - Python **相邻字符串字面量自动拼接**：ref2 生成器里 tran+ic 是一行（两个相邻字符串），
    我抄成两个列表元素 → ic 独立成行 → SFE-874 语法错。抄模式要连引号结构一起抄。
  - **9-bit DAC 扫码 512 溢出**：bit9 无驱动源 → 码 512 与码 0 完全同值（7.7982G 两点全同
    露馅）且污染最小二乘。满量程点用 2^n−1。
  - 折叠锯齿 unwrap 阈值按窗宽算：500p 窗 = 4 phe → 阈值 ±2.5（折叠跳 ~3），不是 ±sat。
  - phe 绝对值含 CKFB 沿固有偏移（+20.7p），静态判定用折边符号+同侧步进差分。
