...
DCO model (actual architecture, user-provided bank definitions):
  ...
  fbank 1b : TEMPORARY single cell of 1 abank LSB on a 1st-order SDM
...
Modeling decisions (cfg-switchable):
 ...
 H. DTC plant per spec table: per-code INL 0.0 LSB RMS (smooth random curve)
    + DNL 0.0 LSB RMS (non-accumulating cell mismatch, see dtc_inl_table)
    and PN floor -300 dBc/Hz @100 MHz = 0 fs RMS white edge jitter
